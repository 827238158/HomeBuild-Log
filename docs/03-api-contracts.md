...
当前已实现：健康、登录、来源列表/详情、附件、审计、当前项目、空间、共享实体、八类正式记录、记录关系，以及本地规则建议和原子批量确认接口。时间线、核心聚合视图、搜索、AI候选和导出恢复仍是未来开发合同。
...
- `GET /timeline`：按日期、精度、阶段、空间和类型查询时间线投影。
- `GET /ledger/summary`：组合采购总额与账目流水计算汇总。
- `GET /issues/board`：按状态返回问题看板投影。
- `GET /spaces/{id}/archive`：返回空间聚合档案。
- `GET /search`：关键词和结构化过滤检索。
