...
访问 `http://127.0.0.1:8000`。当前已验证镜像构建、容器健康、前端首页、SQLite/存储和容器重启持久化；离线包、整机重启、机械硬盘数据目录、局域网访问和真实数据迁移仍待验证。
...
- 当前 Docker 基线默认只允许 Ubuntu 本机访问；需要 Windows 联调时显式绑定家庭局域网地址。
