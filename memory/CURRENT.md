...
## 当前未完成：2026-09-26 本地改动待部署与验收

- 本地已修复记录分析筛选项请求失败的提示与重试，来源列表改由 `App` 单处维护，并将踩坑分析、语音转写提示词移到独立 UTF-8 文本文件。前端全量 131 项测试、相关后端 13 项测试、前端类型检查与构建、修改后端文件的 Ruff 检查均通过；尚未部署到 Ubuntu Docker 或做实机验收。Ubuntu 运行状态仍按下方 2026-09-25 的最近确认记录。
