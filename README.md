...
http://127.0.0.1:8000
```
首次创建配置文件时会生成随机管理员密码并打印在启动日志中，请立即妥善保存；管理员密码以哈希形式保存在 `secrets.json` 中。程序仅在整个配置文件不存在时生成初始密码，删除文件中的密码字段不会重置密码，反而可能导致登录失败。遗失密码时应先备份配置，再单独处理管理员凭据。

## 界面预览

> 产品截图将在完成隐私脱敏后补充。展示装修概览、来源录入、账本和时间线页面。
...
- Windows 11 本地运行；
- Ubuntu 26.04 x86_64 已完成真实数据 Docker 部署和家庭局域网访问验证；最近一次确认记录见 [当前状态](memory/CURRENT.md)。

仍待实机验收：

- 原手机和平板浏览器的响应式布局和日期控件、手机真实登录、Ubuntu 整机重启恢复、机械硬盘运行和资源占用。

尚未作为已验收能力：OCR、图片内容理解、完整 PWA 和离线同步。
